...
handle_interrupt() {
  ...
  echo "[ff] received ${signal_name}. Stopping active jobs..." >&2
  for idx in "${!GPU_LIST[@]}"; do
    pid="${SLOT_PIDS[$idx]:-}"
...
for config_path in "${CONFIGS[@]}"; do
  ...
  if [[ "$slot_index" == "-1" ]]; then
    echo "[ff] internal scheduling error: no free GPU slot found." >&2
    exit 1
  fi
...
if wait_all; then
  echo "[ff] all jobs completed successfully."
  exit 0
fi

echo "[ff] one or more jobs failed. Inspect logs under $LOG_ROOT." >&2
exit 1
